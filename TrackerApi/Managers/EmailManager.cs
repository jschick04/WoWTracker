    ...
    public class EmailManager : IEmailManager {
        ...
        public void SendVerificationEmail(User user, string origin) {
            ...
            if (!string.IsNullOrEmpty(origin)) {
                var verifyUrl = $"{origin}{ApiRoutes.Identity.VerifyEmail}?token={user.VerificationToken}";

                message = $@"<p>Please click the below link to verify your email address.</p>
                             <p><a href=""{verifyUrl}"">{verifyUrl}</a></p>";
            } else {
                message =
                    $@"<p>Please use the below token to verify your email with the <code>{ApiRoutes.Identity.VerifyEmail}</code> api route.</p>
